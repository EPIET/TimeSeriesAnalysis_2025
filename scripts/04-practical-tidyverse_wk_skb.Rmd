...
Import the `mortagg.Rdata` dataset.
...
on the position/sequence of the argumetns of this function.)
...
period. `MA5c` is placed one step forward. 
...
ggplot(mortz, mapping = aes(x = date_index, y = cases)) +
  ...
  labs(x = "Year-Week", 
       y = "Number of deaths", 
       title = "Spain: number of deaths - loess smoothing") +
  tsa_theme
```


In ggplot(), we can also use `stat_smooth`, which provides a smoothing curve 
using a LOESS method. We can change the degree of smoothing by adding a 
...
(outcome), in this case, the number (count) of registered deaths. 
...
Using regression against time is a very simple way to look at the trends. We 
will use the  `glm` function to fit a linear Poisson regression model. Time 
will be respresented by a variable that simply enumerates the observations; 
...
Plot the fitted values against the observed ones. Although the model used 
`index`, in the plot we use instead `date_index` since it is more 
informative. 
...
the offset, in this case for the population, will be forced to be 1; in other 
words, the offset will not affect the outcome. The advantage is that we can 
interpret the IRR of time in terms of mortality rate = cases/population. 
...
```
Compare the trend in the model with and without population.


For communication, it might be better to present the results in terms of 
number of deaths per 1000 inhabitants (population), which would then be 
called the mortality rate. We will now plot the same trend but in terms of 
1000 inhabitants. For this purpose we must adjust both the observed and the 
fitted values. 
...
## Task 4.4 (Optional) {-#task-4-4}

Carry out the same analysis for men and women. Do you see differences?
