...
```{r, knitr-global-chunk-7t, include=FALSE, cache=FALSE}
source("_common.R")
```
...
Your boss is now worried that the analysis you produced in Task 7.1 isn’t right
as it doesn’t account for previously observed excess mortality. She thus asks
...
source("src/tidyverse/session_7.R")
```
...
fitted <- predict(sine2cos2trendmodel)

mortz_fit <- bind_cols(mortz,
    fit = fitted
)
...
mortz_fit <-
    mortz_fit %>%
    mutate(
        difference = cases - fit,
        cusum = cumsum(difference)
    )
...
ggplot(data = mortz_fit) +
    geom_line(
        mapping = aes(x = date_index, y = cusum),
        colour = "orange",
        alpha = 0.7,
        lwd = 2
    ) +
    scale_x_yearweek(date_labels = "%Y-%W", date_breaks = "1 year") +
    labs(x = "Year", y = "Cumulative Sum") +
    tsa_theme()
```
...
residuals and plot the expected cases for 2010 with prediction intervals. The
solution provided will incorporate the seasonality into the model directly and
not use the `predintplot` function of Task 7. 
