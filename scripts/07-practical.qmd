...
ggplot(data = mortz) +
    geom_line(
        mapping = aes(x = date_index, y = cases),
        colour = "black",
        alpha = 1.2
    ) +
    geom_line(
        mapping = aes(x = date_index, 
                      y = mort_trendsin2cos2$fitted),
        colour = "red",
        alpha = 1.4
    ) +
...
pred.df <-
    tibble(
        ...
        date_index = make_yearweek(year = year, week = week),
...
2020 assuming the same Poisson regression model as before.* 

Predict and plot the expected number of deaths per week for 2020 with 
prediction intervals: 

Apply mort_trendsin2cos2 to new data and predict cases with C.I. using 
bootstrapping. 

Bootstrapping is a statistical method where you draw random samples from your 
data, and analyze each of this samples. 

 https://en.wikipedia.org/wiki/Bootstrapping_(statistics)

nSims indicates how many random draws should be carried out; more is better, 
but be aware that it also takes more time. Usually 500 to 1000 is enough. 
...
Plot the forecasted values for the weekly deaths in 2020, and include the 
predictive intervals. 

```{r, task-7-1-prediction-interval-plot-tidy}
ggplot(data = pred.mort) +
    geom_line(
        mapping = aes(x = date_index, y = pred_cases),
        colour = "red",
        alpha = 0.7
    ) +
    geom_ribbon(
        mapping = aes(x = date_index, ymin = lPI, ymax = uPI),
        fill = "red",
        alpha = 0.1
    ) +
...
*Calculate the total number of expected deaths in 2020 in Spain, including 
...
know during which periods of 2020 did Spain experience excess deaths (more 
deaths than what was expected). 
...
# add actual deaths from 2020 to pred.mort
pred.mort <- pred.mort %>% 
...
*Calculate the total excess number of deaths per week, and plot the accumulated number by week.*

Excess deaths can be calculated as the difference of the actual number of fatalities per week and the predicted mean, or the predicted upper 95% PI. We use cumsum() to accumulate the number of deaths over the year.
...
ggplot(data = pred.mort) +
    geom_line(
        mapping = aes(x = date_index, y = cusum_excess_mean),
        colour = "orange",
        alpha = 0.7,
        lwd = 2
    ) +
    geom_line(
        mapping = aes(x = date_index, y = cusum_excess_uPI),
        colour = "blue",
        alpha = 0.7,
        lwd = 2
    ) +   
...
*Interpret the plot. When did the excess number of deaths increase most? What 
does negative excess number of deaths mean?* 

## Task 7.4 (Optional) {#task-7-4 .unnumbered}

*Plot the entire series of observed data from 2010 to 2023, and the fitted 
number from the model based on 2010-2019.*

*Visually, do you think the mortality pattern has gone back to the same 
pattern as in 2010-2019?* 

*How would you determine if the model for 2010-2019 fits well 2021-2023?
...
ggplot(data = mort_2010_2023) +
    ...
    geom_line(data=mortz,
        mapping = aes(x = date_index, y = fitted(mort_trendsin2cos2)),
        colour = "blue",
        lwd=1.0,
        alpha = 0.7
    ) +
...
Carry out the same analysis for men and women. What was the total number of 
...
If you already have the model for each group from a previous practice, use that. If you did not do it before, you can assume the same model as here.
