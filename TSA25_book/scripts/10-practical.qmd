...
Let's check for overdispersion. We can calculate an overdispersion parameter from a model as the ratio of the deviance and the residuals degrees of freedom. Also, proper statistical test are availibale, for example using the `AER` library and the `dispersiontest` function
